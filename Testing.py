...
def test_net(args):
    ...
    for test_dir_img in test_paths:
        ...
        for i, data_batch in enumerate(test_loader):
            ...
            # save saliency maps
            save_test_path = args.save_test_path_root + dataset + '/sod_image'
            if not os.path.exists(save_test_path):
                os.makedirs(save_test_path)
            output_s.save(os.path.join(save_test_path, filename + '.png'))

        cost = np.mean(time_list) * 1000
        cost = cost.item()
        print('dataset: {}, single image cost: {:.1f}ms'.format(test_dir_img.split('/')[0], cost))
